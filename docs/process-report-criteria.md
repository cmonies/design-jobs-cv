...
What community reports are allowed onto job and company pages.

**Where reports live now:** the `reports` table in Supabase. Structured fields publish
immediately after automated checks. Two things wait for a human, both in the Supabase
dashboard: rows with `status = 'held'` (`held_reason` says why — flip to `published` or
`removed`), and any free-text `notes`, which stay hidden until `notes_status` is set to
`approved` (or `rejected`). The rules below are what "approved" means. The whole point of this feature is transparency **without**
